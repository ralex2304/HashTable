...
Для всех вариантов кроме одного размер таблицы `773` (load factor `7`). Такой load factor выбран специально для сравнения функций, так как при меньшей величине будет хуже видна неравномерность распределения.
