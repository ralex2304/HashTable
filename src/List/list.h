...
typedef HashTableItem Elem_t;


#define ELEM_T_PRINTF "Key: %s<br>Val: %zu<br>Hash: %x"
static_assert(sizeof(Elem_t::val)  == 8);
static_assert(sizeof(Elem_t::hash) == 4);


struct ListNode {
    static constexpr Elem_t POISON = {nullptr, 0, 0};    //< poison value
...
#ifndef NDEBUG
inline bool Elem_t_is_poison(Elem_t elem) {
    return elem.hash == ListNode::POISON.hash &&
           elem.val  == ListNode::POISON.val  &&
           elem.key  == ListNode::POISON.key;
}
#endif //< #infdef NDEBUG
