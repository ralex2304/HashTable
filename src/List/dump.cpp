...
void list_dump(const List* list, const VarCodeData call_data) {
    ...
    for (ssize_t i = 0; i < list->capacity; i++) {
        Elem_t elem = list->arr[i].elem;

        LOG_("        ""%3zd | %4zd | %4zd | " ELEM_T_PRINTF "\n",
             i, list->arr[i].prev, list->arr[i].next, elem.key, elem.val, elem.hash);
    }
    ...
    LIST_FOREACH(*list, phys_i, log_i) {
        Elem_t elem = list->arr[phys_i].elem;

        LOG_(" " ELEM_T_PRINTF, elem.key, elem.val, elem.hash);
    }
    LOG_("\n    Physical indexes:");
...
bool list_dump_dot(const List* list, char* img_filename) {
    ...
    for (ssize_t phys_i = 1; phys_i < list->capacity; phys_i++) {
        ...
        if (Elem_t_is_poison(list->arr[phys_i].elem)) {
            FPRINTF_("<tr><td colspan=\"2\">elem = PZN</td></tr>\n");
        } else {
            Elem_t elem = list->arr[phys_i].elem;

            FPRINTF_("<tr><td colspan=\"2\">elem = " ELEM_T_PRINTF "</td></tr>\n",
                     elem.key, elem.val, elem.hash);
        }

        FPRINTF_("<tr><td>prev = %zd </td><td>next = %zd</td></tr></table>>",
                 list->arr[phys_i].prev, list->arr[phys_i].next);
